...
export const updatePlanDayTool = tool({
  name: "updatePlanDay",
  description: `Update an existing plan day's meaningful label. 
  ...
  IMPORTANT:
  - Always use a meaningful label for the plan day by seeing the exercises present inside a plan day. call getPlan to get everything.
  ...
  - Never use the same label for two different plan days. 
  `,
  parameters: z.object({
    planDayId: z.string(),
    label: z
      ...
      .max(100)
      .refine(
        (label) => !/^day\s*\d+(?:\s*-\s*day\s*\d+)?$/i.test(label.trim()),
        {
          message:
            "Use a meaningful workout label, not a generic day label or day number.",
        },
      )
      .optional(),
  }),
