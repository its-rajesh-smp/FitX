    ...
    <section className="relative border-t">
      ...
      <div className="mx-auto grid min-h-[calc(100svh-4rem)] max-w-7xl items-center gap-14 px-4 py-14 sm:px-8 lg:grid-cols-[0.85fr_1.15fr] lg:py-16 [@media(max-height:900px)]:lg:gap-10 [@media(max-height:900px)]:lg:py-8">
        ...
        >
          ...
          <motion.p
            variants={heroItem}
            className="text-muted-foreground mt-6 max-w-xl text-base leading-7 sm:text-lg sm:leading-8 [@media(max-height:900px)]:lg:mt-4"
          >
            FitX creates a practical weekly routine around your goals, time,
            equipment, and experience, then helps you adapt it through
            conversation.
          </motion.p>
          <motion.div
            variants={heroItem}
            className="mt-8 flex flex-col gap-3 sm:flex-row [@media(max-height:900px)]:lg:mt-6"
          >
            <Button
              asChild
              size="lg"
              className="shadow-primary/20 h-12 rounded-xl px-6 text-sm shadow-xl"
            >
              <Link to={primaryHref}>
                {isAuthenticated ? "Continue training" : "Create my free plan"}{" "}
                <ArrowRight />
              </Link>
            </Button>
            <Button
...
export function HighlightsSection() {
  return (
    <section className="bg-canvas/70 border-y">
    ...
    >
      <div className="max-w-2xl">
        ...
        <p className="text-muted-foreground mt-4 text-base leading-7">
          Everything is designed to get you from “I should work out” to knowing
      ...
      <div className="mt-12 grid gap-5 md:grid-cols-3">
        {benefits.map(({ icon: Icon, title, text }, index) => (
          <motion.article
            key={title}
            className="group shadow-card hover:border-primary/25 rounded-3xl border bg-white p-7 transition duration-300 hover:-translate-y-1 hover:shadow-xl"
          ...
          >
            <div className="flex items-center justify-between">
              <span className="bg-primary-soft text-primary group-hover:bg-primary flex size-11 items-center justify-center rounded-2xl transition group-hover:text-white">
  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-7xl gap-14 px-4 sm:px-8 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          <span className="bg-primary inline-flex size-12 items-center justify-center rounded-2xl">
            <Flame className="size-6" />
          </span>
          <p className="mt-8 text-xs font-bold tracking-[0.2em] text-white/50">
            HOW IT WORKS
          </p>
          ...
          <p className="mt-5 max-w-md text-sm leading-7 text-white/60">
            No complicated setup. FitX learns what matters, builds your routine,
            and stays available when things change.
          </p>
        </div>
...
export function StartSection({
  ...
  return (
    <section id="start" className="px-4 py-20 sm:px-8 sm:py-28">
      <div className="bg-primary shadow-primary/25 relative mx-auto max-w-6xl overflow-hidden rounded-[2rem] px-6 py-14 text-center text-white shadow-2xl sm:px-12 sm:py-20">
        ...
        <p className="relative mx-auto mt-5 max-w-xl text-sm leading-7 text-white/75">
          Tell FitX where you are starting. Get a routine you can realistically
          follow.
        </p>
        <Button
          asChild
          size="lg"
          className="text-primary relative mt-8 h-12 rounded-xl bg-white px-6 hover:bg-white/90"
        >
          <Link to={primaryHref}>
            {isAuthenticated ? "Open my program" : "Build my free plan"}{" "}
            <ArrowRight />
          </Link>
        </Button>
      </div>
